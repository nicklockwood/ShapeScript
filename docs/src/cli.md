...
You can download the ShapeScript CLI for Mac or Linux from the [release page](https://github.com/nicklockwood/ShapeScript/releases/tag/{{SHAPESCRIPT_DOCS_VERSION}}), or install it to `$HOME/.local/bin` by running this command in Terminal:

```bash
VERSION={{SHAPESCRIPT_DOCS_VERSION}} /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/nicklockwood/ShapeScript/HEAD/Viewer/CLI/install.sh)"
```
...
```bash
mint install nicklockwood/ShapeScript@{{SHAPESCRIPT_DOCS_VERSION}}
```
