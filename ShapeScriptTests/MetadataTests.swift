...
private func gitTagExists(_ tag: String) throws -> Bool {
    #if os(macOS)
    let process = Process()
    process.currentDirectoryURL = projectDirectory
    process.executableURL = URL(fileURLWithPath: "/usr/bin/env")
    process.arguments = ["git", "rev-parse", "--verify", "--quiet", "refs/tags/\(tag)"]
...
final class MetadataTests: XCTestCase {
    ...
    func exportVersionedHelp() throws {
        ...
        guard shouldUpdateReleaseDocs else {
            XCTAssertTrue(
                try directoriesEqual(tempDirectory, outputDirectory),
                "Help directory for pending release \(projectVersion) is stale. " +
                    "Run ./prepare_release.sh \(projectVersion) to update it."
            )
            return
        }

        if try directoriesEqual(tempDirectory, outputDirectory) {
            return
        }
