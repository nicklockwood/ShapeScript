# Contributing to ShapeScript

Are you interested in contributing to ShapeScript? Fantastic!

Here's what you need to know:

## Versioning

ShapeScript follows the principles of [Semantic Versioning](http://semver.org/spec/v2.0.0.html) (semver). 0.0.x releases are for bug fixes, 0.x.0 releases are for non-breaking or purely additive changes, and x.0.0 releases are for major changes that may break existing code.

Since ShapeScript is an asset creation tool and scripting language implementation as well as a framework, "breaking changes" refers to the interpreted script format and generated geometry as well as the API. Changes to the way scripts are interpreted and/or how geometry is generated by a given function may be considered to be breaking, even if they do not affect the Swift framework API itself.
...
By contributing code to ShapeScript, you are implicitly agreeing to license it under the terms described in the LICENSE.md file. Please do not submit code that you did not write or are not authorized to redistribute.

Inclusion of 3rd party frameworks is **not** permitted, regardless of the license. Small sections of code copied from somewhere else *may* be acceptable, provided that the terms of the original license are compatible with ShapeScript's LICENSE.md, and that you include a comment linking back to the source.

## Code Style

ShapeScript's source code mostly follows the [Ray Wenderlich Style Guide](https://github.com/raywenderlich/swift-style-guide) very closely with the following exception:
...
Tests are run in [Travis CI](https://travis-ci.org/nicklockwood/ShapeScript) automatically on all pull requests, branches and tags. These are the same tests that run in Xcode at development time.
